...
* Per i sistemi Windows, i plug-in PowerShell devono avere il suffisso .ps1, altre finestre
gli script devono avere il suffisso .cmd o .bat e devono essere eseguibili dall'utente
...
Applicazione in uno stato in cui è possibile creare una copia Snapshot. Questo viene chiamato prima dell'operazione di copia Snapshot dello storage.
...
opzionale
 a| 
Questo stub è responsabile dell'esecuzione di tutte le azioni di ripristino. Questo
presuppone che non si stia utilizzando un'interfaccia di ripristino integrata. Viene attivato durante l'esecuzione dell'operazione di ripristino.
